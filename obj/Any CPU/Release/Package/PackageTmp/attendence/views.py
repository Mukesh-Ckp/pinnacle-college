...
def attendance_dashboard(request):
    ...
    student_present_ids = student_attendance_today.filter(status__iexact='present').values_list('student_id', flat=True).distinct()
    student_late_ids = student_attendance_today.filter(status__iexact='late').values_list('student_id', flat=True).distinct()
    student_present_or_late_ids = set(student_present_ids).union(set(student_late_ids))
 
    student_attendance_rate = (len(student_present_or_late_ids) / total_students * 100) if total_students else 0
 
    # ================== Combined Totals ==================
    total_people = total_staff + total_students
    total_present = staff_present + len(set(student_present_ids) - set(student_late_ids))  # ✅ FIXED here
    total_late = staff_late + len(student_late_ids)
    total_present_or_late = staff_present_or_late + len(student_present_or_late_ids)
    ...
    for student in all_students:
        ...
        present_count = StudentAttendance.objects.filter(student=student, status__in=['present', 'late']).count()
        ...
        if total_classes > 0:
            ...
            if percentage < 75:
                low_attendance_students.append({
                    ...
                    'roll_no': student.get_admission_no(),
...
def employee_attendance_list(request):
    # Start by selecting related employees for efficiency
    ...
    # Filtering by category (Teaching or Non-Teaching Staff)
    filter_category = request.GET.get('category')
    if filter_category:
        records = records.filter(employee__category=filter_category)

    # Filtering by employment type (Full-time or Part-time)
    ...
    # Filtering by designation (if you want to filter by position)
    ...
    # Get all unique categories, employment types, and designations for the dropdowns
    ...
    return render(request, 'attendence/attendance_list.html', {
        'records': records,
        'form': form,
        'categories': categories,  # For category dropdown in template
        'employment_types': employment_types,  # For employment type dropdown in template
        'designations': designations,  # For designation dropdown in template
    })
...
def attendance_settings_view(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(
                request,
                f"Check-in time updated successfully to {formatted_time}."
            )
...
def student_attendance_list(request):
    ...
    if subject_filter:
        try:
            attendance_records = attendance_records.filter(subject_id=int(subject_filter))
        except ValueError:
            messages.error(request, f"Invalid subject ID: {subject_filter}")
    if date_filter:
            ...
            try:
                ...
                if attendance_date_obj > today:
                    ...
                    if form_action == "submit_attendance" and selected_timeslot:
                        ...
                        updated_count = 0  # ✅ Track updated students for message

...
def fee_declaration_add(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid() and formset.is_valid():
            ...
            if duplicate:
                form.add_error(None, "This fee declaration already exists.")
            else:
...
def fee_collection_collect(request):
    ...
    for detail in declaration_details:
        ...
        balance = fee_amount - paid_total - discount_total
    ...
    optional_fees = OptionalFee.objects.filter(
        admission_no=admission.admission_no
    ).exclude(fee_type=scholarship_fee_type)
    for opt_fee in optional_fees:
        ...
        balance = opt_fee.amount - paid_total
...
def generate_receipt(request, admission_no):
    logger.info(f"Generating receipt for admission number: {admission_no}")
    admission_no = unquote(admission_no).split(" - ")[0].strip()
    logger.debug(f"Cleaned admission number: {admission_no}")

    # Fetch admission details
    admission = PUAdmission.objects.filter(admission_no__iexact=admission_no).first()
    if not admission:
        logger.info("PUAdmission not found, checking DegreeAdmission.")
        admission = get_object_or_404(DegreeAdmission, admission_no__iexact=admission_no)
    else:
        logger.info(f"PUAdmission found: {admission.admission_no}")
    ...
    logger.debug(f"Student DB record: {student_db}")

    # Use timezone-aware local date
    today = localdate()
    logger.info(f"Filtering today's fee collections (date: {today})")

    fee_collections_today = StudentFeeCollection.objects.filter(
        admission_no=admission.admission_no,
        paid_amount__gt=0,
        payment_date=today
    ).order_by('fee_type__name')

    logger.debug(f"Today's fee collection count: {fee_collections_today.count()}")
    for fee in fee_collections_today:
        logger.debug(
            f" - Fee Type: {fee.fee_type.name}, Paid: {fee.paid_amount}, Payment Date: {fee.payment_date}"
        )

    total_paid_today = sum(fee.paid_amount for fee in fee_collections_today)
    logger.info(f"Total amount paid today: ₹{total_paid_today}")
    ...
    context = {
        'admission': admission,
        'student': student,
        'fee_collections': fee_collections_today,
        'total_paid': total_paid_today,
        'now': timezone.now()
    }
    ...
    logger.info(f"Receipt generated successfully for admission number: {admission.admission_no}")
    return response

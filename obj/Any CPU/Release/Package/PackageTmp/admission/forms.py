...
class DegreeAdmissionForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for field_name, field in self.fields.items():
            if not isinstance(field.widget, (forms.FileInput, forms.CheckboxSelectMultiple, forms.CheckboxInput)):
                current_class = field.widget.attrs.get('class', '')
                if 'form-control' not in current_class:
                    field.widget.attrs['class'] = f"{current_class} form-control".strip()

        # Load all CourseTypes
        self.fields['course_type'].queryset = CourseType.objects.all()

        # Get default CourseType
        default_course_type = CourseType.objects.filter(
            Q(name__icontains="School of Commerce") |
            Q(name__icontains="Commerce") |
            Q(name__icontains="BCom")
        ...
        # Set default course_type only when no data/initial/instance is present
        if (
            default_course_type
            and not self.data
            and not self.initial.get('course_type')
        ...
        # Determine course_type_id for filtering Course and Admitted_to
        course_type_id = None
        if 'course_type' in self.data:
            try:
                course_type_id = int(self.data.get('course_type'))
        ...
        # Add frontend validation to phone number fields
        ...
        fields_to_capitalize = [
            'student_name', 'father_name', 'religion', 'nationality', 'permanent_address', 'current_address'
        ]
        for field in fields_to_capitalize:
            if field in self.fields:
...
class Enquiry1Form(forms.ModelForm):
    ...
    class Meta:
        ...
        fields = [
            ...
            'email', 'source', 'other_source', 'enquiry_date',  # ✅ Added here
        ]
        widgets = {
...
from django import forms
from .models import Enquiry2, Course, CourseType, AcademicYear  # ✅ Ensure AcademicYear is imported
from django.utils import timezone

...
def student_dashboard(request):
    student_userid = request.COOKIES.get('student_userid')
    if not student_userid:
        return redirect('student_login_view')  # Redirect to login if not authenticated
...
def my_assignments_view(request):
    ...
    course_type = student.course_type
    course = student.course
    semester = student.semester or student.current_year  # PU or Degree program

    # Fetch assignments for student based on academic details
...
def submitted_assignments(request, assignment_id):
    ...
    submitted_count = submissions.exclude(submitted_file='').exclude(submitted_file=None).count()

    # ✅ Adjusted filter using subject's course and semester
    total_students = StudentDatabase.objects.filter(
    ...
    if request.method == 'POST':
        for submission in submissions:
            score_val = request.POST.get(f'score_{submission.id}', '').strip()
            remarks_val = request.POST.get(f'remarks_{submission.id}', '').strip()

            # ✅ Explicitly handle blank scores
            if score_val == '':
                ...
                try:
                    ...
                    submission.score = None  # or skip updating score

            # ✅ Handle empty remarks
            submission.remarks = remarks_val if remarks_val != '' else None
